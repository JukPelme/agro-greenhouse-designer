...
> **Замечание о точности правил.** Формулировки в `rules.yaml` написаны «своими словами» — некоторые `rule_id` могут не точно соответствовать пунктам реального СП 107.13330 (в первой версии rules были набросаны до индексации PDF). Validator всё равно возвращает дословную цитату по точному номеру пункта через метаданные ChromaDB. Полная сверка summary↔пункт планируется во второй итерации (LLM-промпт по индексированному тексту).
...
Designer честно фиксирует невыполнимость в обосновании варианта вместо того чтобы выдать правдоподобное-но-ложное решение. После трёх неудачных итераций пересмотра граф останавливается и выпускает отчёт со всеми замечаниями.
...
Готовое веб-демо разворачивается на [Streamlit Cloud](https://share.streamlit.io) из этого же репо:

1. Зайти на [share.streamlit.io](https://share.streamlit.io) и войти через GitHub
2. New app → выбрать репо `JukPelme/agro-greenhouse-designer`
3. Branch: `main`, Main file: `ui/app.py`, Python: `3.12`
4. Deploy — через 2-3 минуты будет URL вида `agro-greenhouse-designer.streamlit.app`

В Settings → Secrets можно опционально положить `ANTHROPIC_API_KEY` — тогда работает live-режим. Без ключа доступны два демо-режима: «норма» и «отказ».
