...
flowchart TD
    ...
    subgraph Knowledge[Знания]
        Rules[(rules.yaml<br/>24 машинопроверяемых<br/>правила: 20 СП + 4 ENG)]
...
Designer пытается ужать блоки до размеров, физически вмещающихся в 500 м², но при этом вынужден сжимать площадь ограждений и подсобок ниже разумного. Validator фиксирует реальные нарушения:

- `WARNING SP107.7.18-south` — площадь проёмов естественной вентиляции < 20% площади ограждения (п. 7.18, цитата из PDF).
- `WARNING ENG.2-aux-share` — подсобные зоны составляют <5% комплекса (sanity-проверка).

Designer честно фиксирует противоречия в обосновании варианта вместо того чтобы выдать правдоподобное-но-ложное решение. Каждое предупреждение сопровождается дословной цитатой соответствующего пункта СП через RAG. См. полный отчёт в [docs/failed_example.md](docs/failed_example.md).
