...
Designer на первой итерации пытается уложиться в физически невозможные ограничения. Validator ловит нарушения, Designer **итеративно адаптирует** компоновку, оставаясь в рамках СП:

- v1 → ERROR SP107.4.4-year-round (расстояние между блоками <6 м) + WARNING ENG.2-aux-share (мало подсобок)
- v2 → Designer увеличивает block_spacing_m до 10 м и долю подсобок до 16% (в обоснование явно вписывает что и зачем поправил)
- v2 → остаётся `WARNING SP107.7.18-south` — площадь проёмов вентиляции < 20% площади ограждения (физически неизбежно на 500 м² при двух блоках)
